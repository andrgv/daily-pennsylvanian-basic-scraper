...
I have changed the scraper to extract the top 6 most read articles every day, which are displyayed on the landing page of thedp.com. To find the target element, I used beautiful soup's select to target the links in the sidebar story CSS element in the website, which contains the top articles. Then I iterated through the target element, extracting each of the 6 links and storing them in a list that I return.
